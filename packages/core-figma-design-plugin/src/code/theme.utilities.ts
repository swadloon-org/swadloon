...
function applyTheme(applyTo) {
  ...
  if (nodes) {
    ...
    let colorStyles = [
      ...new Set(
        jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'PAINT')
      ),
    ];
    let textStyles = [
      ...new Set(
        jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'TEXT')
      ),
    ];
    let effectStyles = [
      ...new Set(
        jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'EFFECT')
      ),
    ];
